...
private void validateRawInput(String str_amount, String str_senderAccountID, String str_receiverAccountID) throws InputException, NumberFormatException, WebApplicationException {
    try{
                if(str_receiverAccountID.isEmpty()) throw new InputException("Receiver invalid");                
                if(str_amount.isEmpty()) throw new InputException(msg+ "Amount invalid");
                if(str_senderAccountID.isEmpty()) throw new InputException("Sender invalid");
                ...
                if(amount<=0) throw new InputException(msg+ "Payment cannot be 0");
                String[] splitter = str_amount.toString().split("\\.");
                if(splitter.length>1&&splitter[1].length()>=3) throw new InputException(msg+" Correct format is two decimal places");
    ...
    private void validatePaymentInput(Account receiverAccount, Account senderAccount, int receiverAccountID, int senderAccountID, double amount)
                                      throws InputException, NumberFormatException, WebApplicationException{
           if(receiverAccount==null) throw new NotFoundException(msg.concat("Receiver account: "+receiverAccount+" does not exist"));
           if(senderAccount==null) throw new NotFoundException(msg.concat("Sender account: "+senderAccount+" does not exist"));
            if(receiverAccountID==senderAccountID) throw new NotFoundException(msg.concat("Sender and Receiver cannot be the same"));
